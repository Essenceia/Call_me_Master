...
void init_by_type(connection_base *toinit){
   ...
   printf("INFO_%d:New client created with NAME %s",pthread_self(),(char *)toinit->rev_msg->msg+1);
...
connection_base* init_connection(int dest_sochket){
    ...
    returnstruct = NULL;
    //default init
    printf("INFO_%d_:Connection init start",pthread_self());
    cbase->alive = DEAD;
    cbase->rev_msg = NULL;
    cbase->handler = NULL;
        ...
        if(cbase->rev_msg != NULL){
            ...
            if(cbase->rev_msg->type == CONNECT){
                ...
                printf("WARN_%d:Message recived from client is of the wrong type"
                             "expecting connect 0x01 but got %x ",pthread_self(),cbase->rev_msg->type);
            }
            //destroy message
...
void destroy_connection_base(connection_base *tokill){
    if(tokill->alive){
    unregister_client(tokill->client_type);
