...
static u_int8_t check_minim_move_valide(CLIENT_LIST player, int8_t x, int8_t y) {
    ...
    for (int8_t dx = -1; dx < 2; dx++) {
        for (int8_t dy = -1; dy < 2; dy++) {
            if (!(dx == 0 && dy == 0)) {
                if (x + dx < get_size_x() && y + dy < get_size_y()) {
                    if (show_at_value(x + dx, y + dx) != EMPTY && flip(0, player, x, y, dx, dy)) {
                        valide = 1;
                        printf("INFO_:Found valide move on x%x y %x\n", x, y);
#ifdef DEBUG_3
...
#endif
            if ((show_at_value((u_int8_t) x, (u_int8_t) y)) == EMPTY) {
